...
XCHAR	addressTemp;
    ...
    if(translatedMsg == RB_MSG_CHECKED){
		...
		if(pRb->ID < BackLightSelectMax)
			...
			if(pRb->ID == AdjustSelectMax-1)
			EEPROMWriteWord(1000, AdjustTimesDataAddress);// add by Spring.Chen
			else
			EEPROMWriteWord((pRb->ID-AdjustSelectMin)*200+300, AdjustTimesDataAddress);// add by Spring.Chen
			}
		...
		else if(pRb->ID < BlowPressMax)
		{
			...
			EEPROMWriteWord((pRb->ID -41 + 1)*5, BlowPressDataAddress);// add by Spring.Chen( 41 Îªµ¥Ñ¡¿ò¿ªÊ¼°´Å¥)
		}
