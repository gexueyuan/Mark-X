...
{
if((AD_press<=epPress )||(AD_press<=Temp_adc ))
    press=0;
else     press=1;
...
unsigned int prepare_and_Charge(void)
{
  ...
  ChargeCount  = 0x0000;
  charge_flag  =0;
  if(charge_Temp1 <= 0x06b2)//960,960 old is 960
    { 
        charge_Temp2 = 0; 
		
                while((charge_Temp2<=0x06b2)&&(ChargeCount <= 5000))//&&(ChargeCount <= 5000))// ,0x0960
             {                                   //2380 ,0x0900
                pump_pwr=1;  
...

{
  ...
  // delayms(100);
   ...
   if(charge_Temp1 <= 0x0960)//2000=7D0
     { 
...
EEPROMWriteWord(peak_compare_time,DebugADDRESS+5);
   ...
   if(flag)
     {
       ...
       pump_reset = 0;
        ...
        temp_sum=sum;
        sum=0;
		EEPROMWriteWord(peak_sample_Count,DebugADDRESS+13);
		EEPROMWriteWord(peak_value,DebugADDRESS+9);
		EEPROMWriteWord(SampleCount,DebugADDRESS+11);
		//EEPROMWriteWord(flag,DebugADDRESS+25);
        temp_sum >>= 5;
		...
		flag=0;
        temp_sum &=0x0000ffff;
		EEPROMWriteWord((unsigned int)temp_sum,DebugADDRESS+7);
...

{
    ...
    unsigned char AdcTemp,ADCResult[4]="8039";

	//Tem_AD =  Adc_Count(7,5);
	test_result= caiyang();

	EEPROMWriteWord(Tem_AD,DebugADDRESS+3);
	cons_t=Cons_Aduj(Tem_AD);
	EEPROMWriteWord(cons_t,DebugADDRESS+15);
	DISICNT = 0x3FFF;  /* disable interrupts */
     ...
     if((test_result != 0)&&(MarkAdc != 0)){
          ...
          if(nflag==0xee)

        	 {
				...
				EEPROMWriteWord(MarkAdc,DebugADDRESS+15);
			    nflag=0;
