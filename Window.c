...
if(GetState(pW,WND_DRAW_CLIENT)){
            ...
            case WND_TITLE_BAR_DRAW:
			   ...
			   Bar(pW->left+1, pW->top,
                        ...
                        // RGB565CONVERT(0x93, 0xEF, 0xFF)
		          for(i=0;i<=25;i++)
                             {	SetColor(RGB565CONVERT(0x93-i,0xef-2*i,0xff-5*i/2));//(0xFF-5*i/2)));//SetColor(RGB565CONVERT(5,70,60));
	                            Bar(pW->left+1, (pW->top+49-i), pW->right-1, (pW->top+50-i));
                              }	
				  i=0;
				  
				  for(i=0;i<=25;i++)
                             {	SetColor(RGB565CONVERT(0x93-i,0xef-2*i,0xff-5*i/2));//(0xFF-5*i/2)));//SetColor(RGB565CONVERT(5,70,60));
	                            Bar(pW->left+1, (pW->top+i), pW->right-1, (pW->top+i+1));
                              }	
            ...
            case WND_TITLE_BAR_BITMAP:
        	    ...
        	    if (pW->pBitmap != NULL){
                    PutImage(pW->left,//+GOL_EMBOSS_SIZE,
                             pW->top+((WND_TITLE_HEIGHT-GetImageHeight(pW->pBitmap))>>1),
